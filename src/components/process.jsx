      ...
      <div className="container process mt-5">
          {data.map((key)=>
            <div className="row gx-2 mb-5 justify-content-center" key={key.id}>
              <div className={`col-12 col-md-6 mb-4 ${ key.id % 2 === 0 ? `order-2 order-lg-2`: `order-1`}`}>
                <span className="process-number text-white">{key.id}</span>
                <h3>{key.title}</h3>
                <p>{key.description}</p>
              </div>
              <div className={`col-12 col-md-4 d-flex justify-content-center ${ key.id % 2 === 0 ? `order-1  order-lg-1`: `order-2`}`}>
